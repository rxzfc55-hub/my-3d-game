...
let buildingVisibilityTimer=0;

function updateNearbyBuildings(dt){
    if(!car || !visibleBuildings.length) return;

    buildingVisibilityTimer += dt;
    if(buildingVisibilityTimer < 0.12) return;
    buildingVisibilityTimer = 0;
    ...
    for(const building of visibleBuildings){
        const dx=building.position.x-cx;
        const dz=building.position.z-cz;
        building.visible = dx*dx + dz*dz <= maxDistSq;
    }
}
...
function animate(){
    ...
    if(
        ...
        if(window.CARGAME_API && Array.isArray(window.CARGAME_API.afterMapDraw)){
            for(const hook of window.CARGAME_API.afterMapDraw){
                try{ hook(); }catch(error){ console.warn("External map hook error:", error); }
            }
        }
...
/* =========================================================
   ...
   ربط آمن للملفات الخارجية بوظائف اللعبة المحلية.
========================================================= */
...
window.CARGAME_API = {
    ...
    getMapExpanded: function(){ return mapExpanded; },
    afterMapDraw: [],
    addAfterMapDrawHook: function(fn){
        if(typeof fn !== "function") return;
        if(this.afterMapDraw.indexOf(fn) === -1){
            this.afterMapDraw.push(fn);
        }
    }
};
