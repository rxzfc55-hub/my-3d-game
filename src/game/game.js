...
const CONFIG={
    ...
    */
    ...
    /*
       150 كم/س حقيقية على العدّاد.
       الرقم بالمتر/الثانية، والعدّاد يضربه
       في 3.6. وكان 150 م/ث أي 540 كم/س،
       وعندها نصف قطر الانعطاف 67 متراً
       مهما كان المقود حاداً.
    */
    MAX_SPEED:41.7,
...
scene.fog=
    new THREE.Fog(
        /*
           نفس لون أفق السماء بالضبط. كان أغمق منه،
           فتظهر المباني البعيدة المغطاة بالضباب
           كأشباح زرقاء على خلفية أفتح.
        */
        0xe5f7ff,
        50,
        150
...
const camera=
    new THREE.PerspectiveCamera(
        ...
        /*
           حد الرسم = حد الرؤية.
           كان 3000: فكل مباني المدينة وأشجارها
           تُرسم ثم يغطيها الضباب — وعند الاستدارة
           يكنس مخروط الرؤية مئات الكائنات البعيدة
           دفعة واحدة، وهذا سبب الثقل. الآن يُلغى
           رسم كل ما بعد 170م تلقائياً.
        */
        170
...
const sky=
    new THREE.Mesh(
        ...
        new THREE.SphereGeometry(
            /*
               داخل حد الرسم (170). القبة تتبع
               السيارة، فحجمها لا يهم بصرياً،
               وتحجب كل ما خلفها: لا مبانٍ بعد
               150م إطلاقاً.
            */
            150,
            32,
        ...
        new THREE.ShaderMaterial({
            ...
            vertexShader:`
                ...
                void main(){
                    ...
                    /*
                       إحداثيات محلية لا عالمية: القبة
                       تتحرك مع السيارة، والإحداثيات
                       العالمية كانت ستُفسد التدرج كلما
                       ابتعدت السيارة عن مركز المدينة.
                    */
                    vPos=
                    position;
...
/*
   ...
   داخل قبة السماء (150)
   وقبل حد الكاميرا البعيد (170).
*/
const SUN_DISTANCE=130;


/* ارتفاع القرص بعد توحيد المقاس */
/* نفس الحجم الظاهري السابق: 130 × (130/1400) */
const SUN_SIZE=12;
...
function setInteriorView(on){
    ...
    /* الخروج من المنظور الداخلي يُفلت المقود */
    if(!interiorView)
        releaseTouchSteer(null);
...
/* =========================================================
   TOUCH STEERING — التوجيه بلمس المقود
   اسحب إصبعك على المقود يميناً أو يساراً.
   يعمل في المنظور الداخلي فقط، لأن المقود
   لا يظهر إلا هناك.
========================================================= */

/* نصف قطر المقود بالأمتار (القطر 38 سم) */
...
const touchSteer={
    ...
    /* نتتبع إصبعاً واحداً بعينه، فلا يتداخل
       معه إصبع الضغط على زر "أمام" */
    pointerId:null,

    startX:0,

    /* مسافة السحب بالبكسل للوصول إلى أقصى
       انعطاف = نصف قطر المقود على الشاشة */
    rangePx:1,

    /* -1 أقصى اليسار ... +1 أقصى اليمين */
    value:0

};


const wheelCenterScreen=
    new THREE.Vector3();

const wheelRimScreen=
    new THREE.Vector3();


/* مركز المقود ونصف قطره بالبكسل على الشاشة */
function wheelScreenCircle(){

    wheelRig.updateWorldMatrix(
        true,
        false
    );


    wheelCenterScreen.set(
        0,
        0,
        0
    );

    wheelRig.localToWorld(
        wheelCenterScreen
    );

    wheelCenterScreen.project(
        camera
    );
    ...
    const cx=(wheelCenterScreen.x+1)/2*w;
    const cy=(1-wheelCenterScreen.y)/2*h;

    const rx=(wheelRimScreen.x+1)/2*w;
    const ry=(1-wheelRimScreen.y)/2*h;
...
function releaseTouchSteer(e){
    ...
    if(
        e &&
        e.pointerId!==touchSteer.pointerId
    )
        return;
...
/*
   الاستماع على الـ canvas نفسه لا على النافذة:
   الأزرار عناصر فوقه تلتقط لمساتها بنفسها،
   فلا يُحسب الضغط على "أمام" إمساكاً بالمقود.
*/
...
function updatePhysics(dt){
    ...
    */
    ...
    if(
        ...
        /*
           التوجيه باللمس قيمة تناظرية: سحبة
           قصيرة = انعطاف خفيف، وسحبة كاملة =
           نفس قوة زر يمين/يسار.
        */
        if(
            touchSteer.active
        ){
...
function updateAudio(){
    ...
    /*
       تنعيم صوت المحرك:
       - نغمة أخفض ومدى أضيق (كانت 58→265 هرتز).
       - مرشّح أخفض بكثير يقصّ الطنين الحاد للموجة
         المنشارية (كان يمرر حتى 2200 هرتز).
       - قفزة الضغط على "أمام" صغرت إلى الثلث.
       - setTargetAtTime بدل التعيين المباشر: تنزلق
         القيم بنعومة بدل القفز، فتختفي الطقطقة.
    */
...
function updateCamera(dt){
    ...
    */
    ...
    if(touchSteer.active){
        ...
        /*
           المقود يتبع الإصبع مباشرة بلا تنعيم،
           وإلا شعرت أنه ينزلق من يدك.
           سحب لليمين (+) = دوران مع عقارب
           الساعة = قيمة سالبة هنا.
        */
        steerVisual=
            -touchSteer.value;
        ...
        /* بعد الإفلات يعود المقود للمنتصف بنعومة */
        steerVisual+=
    ...
    /*
       كان .001 يعني تخلّف الكاميرا عن
       السيارة نحو سُبع ثانية أثناء اللف،
       فيبدو الانعطاف متأخراً عمّا حدث.
    */
